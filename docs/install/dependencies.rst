...
The following are the tested dependencies needed to use pyPRISM:
    - `Python <http://python.org>`__ 3.12 or later
...
These dependencies are required for building from source
    - `Cython <http://cython.org>`__ >= 3.0.0 (for Cython extension compilation)
...
environments for different use-cases (e.g., basic user vs. developer).
The environments can be created using the following command from root directory
of the `repository <https://github.com/usnistgov/pyprism>`__.
